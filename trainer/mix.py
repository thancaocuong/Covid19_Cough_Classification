...
class Mixup(object):
    ...
    def mix(self, images, gpu = True):
        ...
        mixed_images = self.lam*images + (1 - self.lam)*images[indices, :]
        return mixed_images, labels, labels[indices]

    def loss(self, output, labels_1, labels_2, criterion):
        loss = self.lam*criterion(outputs, labels_1) + (1 - self.lam)*criterion(outputs, labels_2)
        return loss
